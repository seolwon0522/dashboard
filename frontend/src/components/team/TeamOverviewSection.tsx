...
export default function TeamOverviewSection({ projectId, model, settings }: Props) {
  ...
  const operationalInsights = useMemo(() => {
    return model.insights.filter((insight) => insight.activeCount > 0 && insight.sampleIssueCount >= 2)
  }, [model.insights])

  const visibleInsights = useMemo(() => {
    if (!activeAssignee) return operationalInsights
    return operationalInsights.filter((insight) => insight.assignee.id === activeAssignee.id)
  }, [activeAssignee, operationalInsights])

  useEffect(() => {
    if (!selectedInsightKey) return
    const exists = visibleInsights.some((insight) => insight.key === selectedInsightKey)
    if (!exists) setSelectedInsightKey(null)
  ...
  useEffect(() => {
    ...
    const assigneeId = assigneeParam === 'unassigned' ? null : Number(assigneeParam)
    const matchedMember = model.capacity.find((member) => member.assignee.id === assigneeId) ?? null
    ...
    setActiveAssignee(matchedMember.assignee)
    setSelectedInsightKey(matchedMember.assignee.id !== null ? String(matchedMember.assignee.id) : null)
  }, [model.capacity, searchParams])
  ...
  function getMemberActionText(member: DashboardModel['capacity'][number]) {
    if (member.overdueCount > 0) return '지연 이슈부터 확인하고 일정 조정이 필요한지 보세요.'
    if (member.staleCount > 0) return '진행 확인 요청이 가장 빠른 다음 조치입니다.'
    if (member.band === 'stretched') return '작업 분산이나 우선순위 조정이 필요합니다.'
    if (member.dueSoonCount > 0) return '이번 주 마감 항목부터 점검하세요.'
    return '현재 흐름은 안정적입니다.'
  }
  ...
  function handleSelectAssignee(assignee: AssigneeFilter | null) {
    ...
    if (assignee) {
      nextParams.set('assignee', assignee.id === null ? 'unassigned' : String(assignee.id))
    } else {
    ...
    <>
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600">
              누가 먼저 확인 대상인지, 왜 그런지, 어디로 이어서 봐야 하는지 빠르게 정리하는 화면입니다.
            </p>
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              <ScopeBadge kind="full" label="전체 프로젝트 기준" />
              <ScopeBadge kind="advisory" label="후속 점검 안내" />
            </div>
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              <div className="rounded-[16px] border border-white bg-white px-3 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">과부하</div>
                <div className="mt-1 text-lg font-semibold text-slate-950">{stretchedCount}명</div>
              </div>
              <div className="rounded-[16px] border border-white bg-white px-3 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">주의</div>
                <div className="mt-1 text-lg font-semibold text-slate-950">{watchCount}명</div>
              </div>
              <div className="rounded-[16px] border border-white bg-white px-3 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">인사이트</div>
                <div className="mt-1 text-lg font-semibold text-slate-950">{operationalInsights.length}개</div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-4">
            ...
            <div className="mt-1 text-sm text-slate-500">
              {selectedMember ? getMemberActionText(selectedMember) : '왼쪽 작업량 표나 아래 인사이트 카드에서 담당자를 선택하면 여기서 바로 후속 판단을 이어서 볼 수 있습니다.'}
            </div>
            {activeAssignee ? (
              <button
                type="button"
        ...
        <div className="mt-4 grid gap-3 md:hidden">
          {topRiskMembers.map((member) => (
            <button
              ...
              className="rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-4 text-left transition-colors hover:border-slate-300"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-semibold text-slate-950">{member.assignee.name}</div>
                <span className="text-xs font-semibold text-slate-500">담당자 보기</span>
              </div>
      ...
      <div className="grid gap-5 2xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="order-1 space-y-5 xl:order-2">
          <section className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm shadow-slate-200/35">
            <div className="text-sm font-semibold text-slate-950">{selectedMember ? '선택한 담당자 요약' : '팀 후속 점검 요약'}</div>
            {selectedMember ? (
              <div className="mt-4 space-y-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <div className="mt-2 text-sm text-slate-500">활성 {selectedMember.openCount}건 · 진행 중 {selectedMember.inProgressCount}건</div>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <div className="text-xs uppercase tracking-[0.12em] text-slate-400">위험 신호</div>
                    <div className="mt-2 text-base font-semibold text-slate-950">지연 {selectedMember.overdueCount}건 · 정체 {selectedMember.staleCount}건</div>
                    <div className="mt-2 text-sm text-slate-500">최근 업데이트율 {Math.round(selectedMember.recentUpdateRate * 100)}%</div>
                  </div>
                </div>

                <div className="rounded-[24px] border border-amber-100 bg-amber-50 px-4 py-4">
                  <div className="text-xs uppercase tracking-[0.12em] text-amber-700">권장 조치</div>
              ...
              <div className="mt-4 space-y-3">
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <div className="mt-2 text-sm text-slate-500">즉시 분산 검토가 필요한 담당자</div>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <div className="mt-2 text-sm text-slate-500">추가 점검이 필요한 담당자</div>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <div className="text-xs uppercase tracking-[0.12em] text-slate-400">인사이트</div>
                    <div className="mt-2 text-2xl font-semibold text-slate-950">{operationalInsights.length}</div>
                    <div className="mt-2 text-sm text-slate-500">바로 참고할 수 있는 흐름 신호</div>
                  </div>
                </div>

                <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">먼저 볼 담당자</div>
                  <div className="mt-3 space-y-2">
                    {topRiskMembers.map((member) => (
                      ...
                      >
                        ...
                        <div className="text-xs font-semibold text-slate-500">담당자 보기</div>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <GroupedWorkloadChart
            title="담당자별 작업량 비교"
            description="상위 담당자 기준으로 활성, 지연, 정체 이슈를 한 번에 비교합니다."
          ...
          {activeAssignee?.id !== null ? (
            ...
            <section className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm shadow-slate-200/35">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-5 text-sm text-slate-600">
                작업량과 위험 신호를 먼저 고른 뒤, 특정 담당자를 선택하면 이 아래에서 행동 패턴과 근거를 이어서 확인할 수 있습니다.
              </div>
            </section>
          )}
        </div>
