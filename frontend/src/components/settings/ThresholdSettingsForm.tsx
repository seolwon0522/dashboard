...
const SETTING_CONTROLS: Array<{
  ...
  { key: 'staleDays', label: '정체 기준', description: '이 일수 이상 업데이트가 없으면 정체로 봅니다.', min: 3, max: 30 },
  { key: 'dueSoonDays', label: '임박 일정 기준', description: '이 일수 이내 마감이면 확인 대상으로 올립니다.', min: 1, max: 21 },
  { key: 'recentActivityDays', label: '최근 활동 기준', description: '이 일수 이내 갱신이면 최근 활동으로 봅니다.', min: 1, max: 21 },
  { key: 'recentCompletionDays', label: '최근 완료 기준', description: '이 일수 이내 완료를 보너스와 흐름 계산에 씁니다.', min: 1, max: 30 },
  { key: 'overloadThreshold', label: '담당자 과부하 기준', description: '활성 이슈 수가 이 기준을 넘기면 부하를 경고합니다.', min: 1, max: 20 },
  { key: 'longOverdueDays', label: '장기 지연 기준', description: '장기 지연 추가 패널티를 줄 임계값입니다.', min: 3, max: 45 },
]
...
function SettingControlCard({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-sm shadow-slate-200/10">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">기준을 빠르게 바꿉니다.</div>
          </div>
          <button
            type="button"
            onClick={onReset}
      ...
      <section className="space-y-4 rounded-[24px] border border-slate-200 bg-slate-50/80 px-5 py-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs leading-5 text-slate-500">필요할 때만 조정합니다.</div>
          </div>
          <button
            type="button"
        ...
        {showAdvancedWeights ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {WEIGHT_CONTROLS.map((control) => (
              <label key={control.key} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm shadow-slate-200/10">
