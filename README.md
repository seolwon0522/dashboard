...
| 버전 | 날짜 | 주요 변경사항 |
|---|---|---|
| v0.7 | 2026-04-07 | 코드베이스 경량화 — dead code 제거, 중복 통합, 미사용 설정 정리 |
...
### v0.7 — 코드베이스 경량화 및 정리 (2026-04-07)

**삭제 — 미사용 코드 제거**
- `SummaryCard.tsx` 삭제 — KpiRow 내부 KpiCard가 완전히 대체, 어디서도 import하지 않음
- `OverdueTable.tsx` 삭제 — AttentionPanel이 동일 기능 제공, 어디서도 import하지 않음
- `fetchOverdueIssues()` (api.ts) 삭제 — 프론트엔드에서 미호출
- `OverdueIssue`, `OverdueIssuesResponse` (types/dashboard.ts) 삭제 — 위 함수 삭제로 미사용
- `CacheStatsResponse`, `CacheInvalidateResponse` (schemas/dashboard.py) 삭제 — 엔드포인트 미연결
- `fetch_issue_statuses()` (redmine_client.py) 삭제 — 호출처 없는 dead code
- `StaleRule` 클래스 + `stale_rule` 설정 (config.py, config.json) 삭제 — 정의만 있고 어떤 서비스에서도 미사용

**통합 — 중복 제거**
- FilterChips 내부 `STATUS_LABELS` 제거 → `labels.ts`의 `STATUS_GROUP_LABEL` 공유 사용으로 통합
- IssueDetailDrawer 인라인 `<style jsx>` slideIn 애니메이션 제거 → `tailwind.config.ts`에 `slideIn` keyframe 추가하여 `animate-slideIn` 클래스로 통합

**버그 수정**
- `WorkloadService._fetch_project_issues()`에 `subproject_id` 필터링 누락 수정 — IssueService와 동일한 subproject 제외 로직 적용
